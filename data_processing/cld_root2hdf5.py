...
def process_root_files(input_dir, output_dir, events_per_hdf5=None, max_events=None, max_root_files=None):
    ...
    for root_file in tqdm(
        ...
        for iev in range(len(ev["MCParticles.momentum.x"].array())):
            ...
            # print("calo_hit_features", calo_hit_features)
            # print("genparticle_to_calo_hit_matrix", genparticle_to_calo_hit_matrix)
            ...
            event_group.create_dataset(
                "genparticle_to_calo_hit_matrix/rows", data=genparticle_to_calo_hit_matrix["gen_idx"]
            )
            event_group.create_dataset(
                "genparticle_to_calo_hit_matrix/cols", data=genparticle_to_calo_hit_matrix["hit_idx"]
            )
            event_group.create_dataset(
                "genparticle_to_calo_hit_matrix/weights", data=genparticle_to_calo_hit_matrix["weight"]
            )

            event_group.create_dataset(
                "genparticle_to_tracker_hit_matrix/rows", data=genparticle_to_tracker_hit_matrix["gen_idx"]
            )
            event_group.create_dataset(
                "genparticle_to_tracker_hit_matrix/cols", data=genparticle_to_tracker_hit_matrix["hit_idx"]
            )
            event_group.create_dataset(
                "genparticle_to_tracker_hit_matrix/weights", data=genparticle_to_tracker_hit_matrix["weight"]
            )

            event_group.create_dataset("track_to_tracker_hit_matrix/rows", data=track_to_tracker_hit_matrix["track_idx"])
            event_group.create_dataset("track_to_tracker_hit_matrix/cols", data=track_to_tracker_hit_matrix["hit_idx"])
            event_group.create_dataset("track_to_tracker_hit_matrix/weights", data=track_to_tracker_hit_matrix["weight"])

            event_group.create_dataset("cluster_to_cluster_hit_matrix/rows", data=cluster_to_cluster_hit_matrix["cluster_idx"])
            event_group.create_dataset("cluster_to_cluster_hit_matrix/cols", data=cluster_to_cluster_hit_matrix["hit_idx"])
            event_group.create_dataset(
                "cluster_to_cluster_hit_matrix/weights", data=cluster_to_cluster_hit_matrix["weight"]
            )
            ...
            event_group.create_dataset("gp_to_gp/rows", data=gp_to_gp["parent_idx"])
            event_group.create_dataset("gp_to_gp/cols", data=gp_to_gp["daughter_idx"])
            event_group.create_dataset("gp_to_gp/weights", data=gp_to_gp["weight"])
